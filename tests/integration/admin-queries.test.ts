// The admin read models over a small, hand-built history (real Postgres).
...
async function history() {
  const s = shop(t.db);
  const ev = await venue(s.db, { totalSeats: 100, seatsSold: 0, startsAtMs: NOW + 20 * DAY });
...
describe("admin overview", () => {
  it("sums revenue, tickets and refunds for the period and the one before", async () => {
    ...
    const o = await getOverview(s.db, NOW, 7);
...
describe("admin tables", () => {
  it("filters and sorts orders, and finds one by its TB- number", async () => {
    const { s, a, b } = await history();
    expect((await listOrdersAdmin(s.db, {})).total).toBe(3);
    expect((await listOrdersAdmin(s.db, { status: "refunded" })).rows.map((r) => r.order.id)).toEqual([a.id]);
    expect((await listOrdersAdmin(s.db, { q: "b@example" })).rows.map((r) => r.order.id)).toEqual([b.id]);
    expect((await listOrdersAdmin(s.db, { q: `TB-${String(b.id).padStart(5, "0")}` })).rows.map((r) => r.order.id)).toEqual([b.id]);
    const byTotal = (await listOrdersAdmin(s.db, { sort: "total", dir: "desc" })).rows.map((r) => r.order.totalCents);
