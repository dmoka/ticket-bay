// Booking and cancelling through the order services against a real Postgres
// (Testcontainers) and the fake payment provider. Only the clock is under test
// control. The races are real: two checkouts run on two pooled connections and
// queue on the event's row lock.
//
...
describe("a cancellation after the show has started pays nothing", () => {
  ...
  it("pays a no-show nothing while also keeping their seat off the market", async () => {
    ...
    expect((await getEvent(s.db, ev.id))!.seatsSold).toBe(100);
  });
});
...
describe("the refund window, order by order", () => {
  it("pays the customer who cancelled in time and nothing to the one who did not, on the same venue", async () => {
    ...
    expect((await getEvent(s.db, ev.id))!.seatsSold).toBe(98);
  ...
  it("tells the two kinds of zero apart: a fee-swallowed refund frees the seat, a late one does not", async () => {
    const s = shop(t.db);
    const ev = await venue(s.db, { priceCents: 25 });
...
describe("checkout", () => {
  ...
  it("applies the early-bird discount 30+ days out", async () => {
    const s = shop(t.db);
    const ev = await venue(s.db, { startsAtMs: NOW + 40 * DAY });
  ...
  it("does not charge twice when the same checkout is submitted twice", async () => {
    ...
    expect((await getEvent(s.db, ev.id))!.seatsSold).toBe(42);
  });

  it("applies a discount code and counts its use", async () => {
    const s = shop(t.db);
    const ev = await venue(s.db);
    await addCode(s.db, "WELCOME10", 10);
    ...
    expect((await getCode(s.db, "WELCOME10"))!.uses).toBe(1);
  });

  it("refuses an unknown, expired or used-up code without charging", async () => {
    const s = shop(t.db);
    const ev = await venue(s.db);
    await addCode(s.db, "OLD", 25, { expiresAtMs: NOW });
    await addCode(s.db, "ONCE", 50, { maxUses: 1, uses: 1 });
    ...
    expect((await getEvent(s.db, ev.id))!.seatsSold).toBe(40);
  });
  ...
  it("refunds the charge when the last seats go while the card is being charged", async () => {
    const s = shop(t.db, chargesTogether(2));
    const ev = await venue(s.db);
  ...
  it("does not let two checkouts share the last use of a limited code", async () => {
    const s = shop(t.db, chargesTogether(2));
    const ev = await venue(s.db);
    await addCode(s.db, "LAST", 20, { maxUses: 1 });
    ...
    expect((await getCode(s.db, "LAST"))!.uses).toBe(1);
    // The losing checkout's charge was given back in full.
    expect((await getEvent(s.db, ev.id))!.seatsSold).toBe(41);
  });
});
